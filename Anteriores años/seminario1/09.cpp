...
double calcular_integral_secuencial(  )
{
   ...
   for( long j = 0; j < m*(1/4); j++ )            // para cada $j$ entre $0$ y $m-1$:
...
double funcion_hebra( long i ) {
 ...
 double suma=0.0;
 for(long j=m*(i/n); i< m*(i/n) +(m/n); j++){
...
double calcular_integral_concurrente( )
{
 long resultado=0.0;
   ...
   future<double> hebra[n];

   for (int i=0; i<n; i++){
      hebra[i]=async (launch::async, funcion_hebra,i);

   }

   for (int i=0; i<n; i++){
      
      resultado += hebra[i].get();
